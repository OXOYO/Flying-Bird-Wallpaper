...
H5 浏览/搜索网格支持视频缩略图 **内联播放**（hover 或点击触发，视页面配置而定）：

| 能力 | 说明 |
|------|------|
| **静音按钮** | `H5InlineVideoMuteButton.vue`，右下角 Iconify 图标（`custom:volume-on` / `custom:volume-mute`） |
| **显示条件** | 仅当视频 **实际播放中**（`!paused && !ended`）时显示；由 `isInlineVideoActive` 控制 |
| **状态同步** | `useH5ResourceBrowse.mjs` 维护 `inlineVideoMuteByKey`，监听 `<video>` 的 `playing` 事件 |
| **接入页面** | `h5/pages/search/index.vue`、`H5ResourceBrowseView.vue` |

默认内联播放为 **静音**；用户可点击按钮切换，状态按资源 key 独立记忆。

### 敏感内容遮罩（2.0.0+）

与桌面端策略一致：开启 **设置 → 隐私空间 → 敏感内容隐藏** 且已设密码时，对 `nsfwLevel ≥ 2` 的资源在网格中显示 `H5NsfwContentMask.vue` 遮罩；搜索 SQL 本身不过滤敏感项。详见 [privacy-and-sensitive-content.md](./privacy-and-sensitive-content.md)。
