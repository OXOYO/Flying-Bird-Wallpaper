...
1. 打开飞鸟壁纸应用
2. 进入 `设置` → `插件市场` → `已安装`
3. 在需要密钥的插件卡片上点击 `配置密钥`
4. 输入并保存对应 API 密钥
5. 进入 `搜索`，选择对应的资源插件并输入关键词进行测试

> 说明：文档中提到的“远程资源”，在当前版本中就是“资源插件”。
...
### Q: Windows 本地 build:win 在 arm64 步骤失败？

**A:**

默认会打包 x64 + arm64。若日志出现 `MSB8020: 无法找到 v143 的生成工具` 且 `Platform=ARM64`，说明本机 Visual Studio **未安装 ARM64 C++ 生成工具**（与是否关闭 IDE/Electron 无关）。

**可选方案：**

1. **只打 x64（最快）**：`npm run build && electron-builder --win --x64`
2. **补全环境**：Visual Studio Installer → 修改 →「使用 C++ 的桌面开发」→ 勾选 **MSVC v143 - VS 2022 C++ ARM64 生成工具**
3. **CI 打包**：推送 `v*` 标签后 GitHub Actions 会自动双架构打包，无需本地 ARM64 环境

### Q: 列表里视频预览如何静音/取消静音？

**A:**

- **桌面端**：探索页、合集页卡片内视频播放时，右下角显示音量按钮；默认静音，点击可切换。
- **H5 端**：搜索/浏览网格内联播放视频时，右下角同样有静音按钮；仅在视频实际播放时显示。
- **全屏预览**：桌面端全屏播放时关闭按钮在鼠标移入视频区域后显示（键盘聚焦时也会显示）。

### Q: 「敏感内容隐藏」如何工作？

**A:**

2.0.0 起在 **设置 → 隐私空间** 中开启。须先设置隐私密码。开启后：

- 探索/合集/H5 浏览时对 `nsfwLevel ≥ 2` 的资源显示遮罩（搜索列表本身不过滤）
- 自动/快捷键切换下一张壁纸时会跳过敏感项
- 手动「设为壁纸」不受影响

详见 [敏感内容隐藏文档](./privacy-and-sensitive-content.md)。
