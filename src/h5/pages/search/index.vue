...
const {
  ...
  getPageSize: () => resolveSearchPageSize(),
  getDedupKey: getSearchListDedupKey
})
...
const buildSimilarScope = () => buildH5SearchSimilarScope(form)
  ...
  try {
    ...
    if (res?.success && res.data?.list?.length) {
      ...
      list.value = startSimilar({
        resourceId: item.id,
        scope,
        sourceItem: normalizeItem(item),
        firstRows: res.data.list.map(normalizeItem),
        pageSize,
