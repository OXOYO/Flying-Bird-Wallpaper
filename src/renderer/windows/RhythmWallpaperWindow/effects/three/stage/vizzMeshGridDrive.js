...
/** vizz mesh-grid drawFrame 顶点驱动（从 vizz.fm bundle id:"mesh-grid" 还原） */
export function driveVizzMeshGrid(state, spectrum, dt) {
  const {
    positions,
    basePositions,
    frequencies,
  ...
  const dispAxis = state.displaceAxis ?? 1
...
/** 无音频：仍保持 Perlin 流动 + 同心涟漪 */
export function driveVizzMeshGridIdle(state, dt) {
  ...
  for (let e = 0; e < U; e++) {
    ...
    const V = smoothToward(smoothHeights[e], softenDisplacement(target, lift), f * (1 - 0.5 * (params.jaggedness ?? 0.15)), dt)
    smoothHeights[e] = V
  }
